...
export default function App() {
  ...
  // --- İKİLİ VE KALICI DÖNEM YÖNETİMİ ---
  const [satisDonem, setSatisDonem] = useState<string>(() => {
    return localStorage.getItem("satisDonem") || getLocalDateString().substring(0, 7);
  });
  
  const [genelDonem, setGenelDonem] = useState<string>(() => {
    return localStorage.getItem("genelDonem") || getLocalDateString().substring(0, 7);
  });
  
  const aktifDonem = ['satis', 'analiz'].includes(activeTab) ? satisDonem : genelDonem;
  const setAktifDonem = ['satis', 'analiz'].includes(activeTab) ? setSatisDonem : setGenelDonem;

  useEffect(() => { 
    localStorage.setItem("satisDonem", satisDonem); 
  }, [satisDonem]);

  useEffect(() => { 
    localStorage.setItem("genelDonem", genelDonem); 
  }, [genelDonem]);
  ...
  const [sutForm, setSutForm] = useState<SutGiris>({ 
    tarih: bugun, 
    ciftlik: "", 
    kg: "", 
    fiyat: "", 
  ...
  const [fisUst, setFisUst] = useState({ 
    tarih: bugun, 
    bayi: "", 
    aciklama: "", 
    odeme_turu: "PEŞİN", 
  ...
  const [gosterilenEkler, setGosterilenEkler] = useState({ 
    tereyagi: false, 
    yogurt_kaymagi: false, 
    iade: false, 
    bos_kova: false 
  });
  const [sonFisData, setSonFisData] = useState<any>(null);

  const [fisFiltre, setFisFiltre] = useState<{ bayiler: string[], baslangic: string, bitis: string }>({ 
    bayiler: [], 
    baslangic: "", 
    bitis: "" 
  });
  ...
  // --- ANALİZ STATE'LERİ ---
  const [analizFiltre, setAnalizFiltre] = useState<{bayiler: string[], urunler: string[], baslangic: string, bitis: string}>({ 
    bayiler: [], 
    urunler: [], 
    baslangic: "", 
    bitis: "" 
  ...
  const [giderForm, setGiderForm] = useState<Gider>({ 
    tarih: bugun, 
    tur: "Genel Gider", 
    aciklama: "", 
    tutar: "" 
  ...
  const giderTurleri = [
    "Araç Yakıt", "Süt Ödemesi", "Yemek", "Sarf Malzeme", "Genel Gider", 
    "Nakliye", "Maaş", "Araç Bakım", "Elektrik Ödemesi", "Süt Katkıları", 
    "Tamirat Tadilat", "Katı Yakacak", "Sermaye Girişi", "Kar Paylaşımı", 
    "Kova Satışı", "süt nakliye", "yoğurt nakliye", "tahsilat", "banka kesintisi"
  ];
  ...
  const [uretimForm, setUretimForm] = useState<Uretim>({ 
    tarih: bugun, cig_sut: "", sut_fiyat: "", sut_tozu: "", sut_tozu_fiyat: "", 
    tereyag: "", tereyag_fiyat: "", katki_kg: "", katki_fiyat: "", su: "", 
    kova_3_adet: "", kova_3_fiyat: "", kova_5_adet: "", kova_5_fiyat: "", 
    cikti_3kg: "", satis_3_fiyat: "", cikti_5kg: "", satis_5_fiyat: "", aciklama: "" 
  });
  ...
  useEffect(() => { 
    if (session) verileriGetir("hepsi"); 
  }, [session]);
  ...
  async function verileriGetir(hedef: "hepsi" | "satis" | "sut" | "gider" | "uretim" | "ayar" = "hepsi") {
    try {
      if (hedef === "hepsi" || hedef === "ayar") {
        ...
        if (u) {
          ...
          setFisDetay(prev => {
            ...
            u.forEach(urun => { 
              if (!yeniDetay[urun.id]) {
                yeniDetay[urun.id] = { adet: "", kg: "", fiyat: urun.fiyat || "" }; 
              }
            });
      ...
      // SİLİNMEYENLERİ GETİR (Çöp Kutusu Filtresi: silindi_mi !== true)
      if (hedef === "hepsi" || hedef === "satis") {
        ...
        if (f) setSatisFisList(f.filter(x => x.silindi_mi !== true));
        if (st) setSatisList(st.filter(x => x.silindi_mi !== true));
      }
      ...
      if (hedef === "hepsi" || hedef === "sut") {
        ...
        if (s) setSutList(s.filter(x => x.silindi_mi !== true));
      }
      ...
      if (hedef === "hepsi" || hedef === "gider") {
        ...
        if (g) setGiderList(g.filter(x => x.silindi_mi !== true));
      }
      ...
      if (hedef === "hepsi" || hedef === "uretim") {
        ...
        if (ur) setUretimList(ur.filter(x => x.silindi_mi !== true));
      }
...
export default function App() {
  ...
  const handleBayiSecimi = (secilenBayi: string) => {
    ...
    urunler.forEach(u => {
      ...
      if (!editingFisId) {
        yeniDetay[u.id] = { 
          adet: fisDetay[u.id]?.adet || "", 
          kg: fisDetay[u.id]?.kg || "", 
          fiyat: String(hafizaFiyat) 
        };
      }
    });
    ...
    if (!editingFisId) {
        yeniDetay["v_iade"] = { 
          adet: fisDetay["v_iade"]?.adet || "", 
          kg: "", 
          fiyat: hafizaIadeFiyat 
        };
        yeniDetay["v_bos_kova"] = { 
          adet: fisDetay["v_bos_kova"]?.adet || "", 
          kg: "", 
          fiyat: hafizaKovaFiyat 
        };
    }
  ...
  // Güncel bakiye bulma (GENEL BORÇ MANTIĞI)
  ...
  async function handleTopluFisKaydet() {
    if (!fisUst.bayi) {
      return alert("Lütfen bir Bayi/Market seçin!");
    }
    if (!bayiler.some(b => b.isim === fisUst.bayi)) {
      return alert("Lütfen listeden geçerli bir Bayi/Market seçin! Kendiniz rastgele isim giremezsiniz.");
    }
    ...
    if (eklenecekUrunler.length === 0 && iadeMiktar === 0 && kovaMiktar === 0) {
      return alert("Fişte işlem yok! Ürün, iade veya kova girin.");
    }
    ...
    if (editingFisId) {
      ...
      // Eski detayları soft delete yap
      await supabase.from("satis_giris").update({silindi_mi: true}).eq("fis_no", ortakFisNo);
      ...
      if (iadeMiktar > 0) {
        insertArray.push({ 
          fis_no: ortakFisNo, tarih: fisUst.tarih, bayi: fisUst.bayi, urun: "İade", 
          adet: iadeAdet, fiyat: -iadeFiyat, birim: 1, toplam_kg: iadeKg, 
          tutar: -(iadeMiktar * iadeFiyat), bos_kova: 0, aciklama: `Bağlı Fiş: ${ortakFisNo}`, 
          ekleyen: username 
        });
      }
      ...
      if (kovaMiktar > 0) {
        insertArray.push({ 
          fis_no: ortakFisNo, tarih: fisUst.tarih, bayi: fisUst.bayi, urun: "Boş Kova", 
          adet: kovaAdet, fiyat: -kovaFiyat, birim: 1, toplam_kg: kovaKg, 
          tutar: -(kovaMiktar * kovaFiyat), bos_kova: kovaAdet, aciklama: `Bağlı Fiş: ${ortakFisNo}`, 
          ekleyen: username 
        });
      }

      const { error: errDetay } = await supabase.from("satis_giris").insert(insertArray);
      if (errDetay) return alert("Detaylar kaydedilirken hata oluştu!");
      ...
      if (errDetay && savedFisId) {
          // Rollback için hard delete yapıyoruz çünkü yanlış oluştu
    ...
    if (iadeMiktar > 0) {
      ekstraIndirimler.push({ isim: "İade", miktar: iadeMiktar, birim: iadeKg > 0 ? "KG" : "Ad", tutar: iadeMiktar * iadeFiyat });
    }
    if (kovaMiktar > 0) {
      ekstraIndirimler.push({ isim: "Boş Kova", miktar: kovaMiktar, birim: kovaKg > 0 ? "KG" : "Ad", tutar: kovaMiktar * kovaFiyat });
    }
    ...
    const fisGosterimData = {
      ...
      tarih: fisUst.tarih, 
      bayi: fisUst.bayi, 
      aciklama: fisUst.aciklama, 
      teslim_alan: fisUst.teslim_alan,
      ...
      ekstraIndirimler,
      genelToplam: fisCanliToplam, 
      tahsilat: tahsilat, 
      kalanBakiye: (fisCanliToplam - tahsilat), 
      odeme: fisUst.odeme_turu,
      eskiBorc: eskiBorc, 
    ...
    verileriGetir("satis"); 
    setSonFisData(fisGosterimData);
  }

  const resetFisForm = () => {
    setEditingFisId(null); 
    setEditingFisNo(null);
    setFisUst({ 
      tarih: bugun, 
      bayi: "", 
      aciklama: "", 
    ...
    urunler.forEach(u => {
      temizDetay[u.id] = { adet: "", kg: "", fiyat: u.fiyat || "" };
    });
  ...
  const handleYeniFisAc = () => { 
    resetFisForm(); 
    setIsFisModalOpen(true); 
  };
  const handleFisDuzenle = (fis: any) => {
    setEditingFisId(fis.id); 
    setEditingFisNo(fis.fis_no);
    ...
    setFisUst({ 
      ...
      tahsilat: fis.tahsilat > 0 ? String(fis.tahsilat) : "", 
      bos_kova: "", 
      teslim_alan: tAlan 
    });
    ...
    urunler.forEach(u => {
      ...
      dolanDetay[u.id] = { 
        adet: buUrun ? String(buUrun.adet) : "", 
        kg: calculatedKg, 
        fiyat: buUrun ? String(buUrun.fiyat) : String(u.fiyat || "") 
      };
    });
  ...
  const handleFisDetayGoster = (fis: SatisFis) => {
    ...
    // Doğru önceki bakiye hesabı (Tarih ve ID bazlı sıralı kontrol)
    ...
    if (iadeUrun) {
      ekstraIndirimler.push({ 
        isim: "İade", 
        miktar: iadeUrun.toplam_kg && Number(iadeUrun.toplam_kg) > 0 ? iadeUrun.toplam_kg : iadeUrun.adet, 
        birim: iadeUrun.toplam_kg && Number(iadeUrun.toplam_kg) > 0 ? "KG" : "Ad", 
        tutar: Math.abs(Number(iadeUrun.tutar)) 
      });
    }
    
    if (kovaUrun) {
      ekstraIndirimler.push({ 
        isim: kovaUrun.urun === "İade Kova" ? "İade Kova" : "Boş Kova", 
        miktar: kovaUrun.toplam_kg && Number(kovaUrun.toplam_kg) > 0 ? kovaUrun.toplam_kg : kovaUrun.adet, 
        birim: kovaUrun.toplam_kg && Number(kovaUrun.toplam_kg) > 0 ? "KG" : "Ad", 
        tutar: Math.abs(Number(kovaUrun.tutar)) 
      });
    }
    ...
    setSonFisData({ 
      ...
      urunler: ilgiliUrunler.map(u => {
          let calculatedKg = 0;
          const a = Number(u.adet);
          const t = Number(u.tutar);
          const f = Number(u.fiyat);
          
          if (f !== 0 && Math.abs(t - (a * f)) > 0.01) calculatedKg = t / f;
          return { isim: u.urun, adet: a, kg: calculatedKg, fiyat: f, tutar: t };
      }), 
      ekstraIndirimler,
      genelToplam: fis.toplam_tutar, 
      tahsilat: fis.tahsilat, 
      kalanBakiye: fis.kalan_bakiye, 
      odeme: fis.odeme_turu || "Bilinmiyor",
      eskiBorc: oGunkuEskiBorc, 
      genelBorc: oGunkuEskiBorc + fis.kalan_bakiye,
      gosterBakiye: false 
    });
  };
  ...
  const handleWhatsappResimGonder = () => {
    ...
    if (typeof (window as any).html2canvas !== "undefined") {
      ...
      alert("Resim oluşturucu yükleniyor, 1 saniye sonra tekrar deneyin.");
    }
  };

  // --- PERFORMANS ODAKLI FİLTRELEME VE TOPLAM HESAPLAMALARI (Döneme Göre) ---
  const periodSatisFis = useMemo(() => satisFisList.filter(f => f.tarih.startsWith(satisDonem)), [satisFisList, satisDonem]);
  const periodSut = useMemo(() => sutList.filter(s => s.tarih.startsWith(genelDonem)), [sutList, genelDonem]);
  const periodGider = useMemo(() => giderList.filter(g => g.tarih.startsWith(genelDonem)), [giderList, genelDonem]);
  const periodUretim = useMemo(() => uretimList.filter(u => u.tarih.startsWith(genelDonem)), [uretimList, genelDonem]);
  const periodSatisList = useMemo(() => satisList.filter(s => s.tarih.startsWith(satisDonem)), [satisList, satisDonem]);

  const fFisList = useMemo(() => sortData(periodSatisFis.filter((f: any) => {
    const bayiMatch = fisFiltre.bayiler.length === 0 || fisFiltre.bayiler.includes(f.bayi);
    const tarihMatch = (!fisFiltre.baslangic || f.tarih >= fisFiltre.baslangic) && (!fisFiltre.bitis || f.tarih <= fisFiltre.bitis);
    const kisiMatch = satisFiltreKisi === 'herkes' || (f.ekleyen && f.ekleyen.toLowerCase() === username.toLowerCase());
    return bayiMatch && tarihMatch && kisiMatch;
  }), fisSort), [periodSatisFis, fisFiltre, fisSort, satisFiltreKisi, username]);
  ...
  const fGiderList = useMemo(() => sortData(periodGider, giderSort), [periodGider, giderSort]);
  ...
  // Tüm Fişlerden Müşteri Borç Durumu Hesaplama (Sistem işlemleri ve silinenler hariç tüm aylar)
  const bayiBorclari = useMemo(() => {
    const borclar: Record<string, number> = {};
    
    satisFisList.forEach(f => {
        if (f.bayi === "SİSTEM İŞLEMİ" || f.silindi_mi) return;
  ...
  // --- EKRAN BİLEŞENLERİ (RENDER) ---

  const renderOzet = () => (
    <div className="tab-fade-in main-content-area">
      <div className="cards-grid">
        <div className="card summary-c" style={{ borderLeft: `5px solid #059669` }}>
          <small>Dönem Satış</small>
          <h2 style={{ margin: "5px 0", color: "#059669" }}>{fSayiNoDec(tFisToplam)} ₺</h2>
        </div>
        <div className="card summary-c" style={{ borderLeft: "5px solid #dc2626" }}>
          <small>Dönem Gider</small>
          <h2 style={{ margin: "5px 0", color: "#dc2626" }}>{fSayiNoDec(genelToplamGider)} ₺</h2>
        </div>
        <div className="card summary-c" style={{ borderLeft: "5px solid #2563eb" }}>
          <small>Tahsilat</small>
          <h2 style={{ margin: "5px 0", color: "#2563eb" }}>{fSayiNoDec(tFisTahsilat)} ₺</h2>
        </div>
        <div className="card summary-c" style={{ borderLeft: "5px solid #f59e0b" }}>
          <small>Bayi Açık Hesap (Genel Net)</small>
          <h2 style={{ margin: "5px 0", color: "#f59e0b" }}>{fSayiNoDec(bayiNetDurum)} ₺</h2>
        </div>
      </div>
      
      <div className="card" style={{marginTop: "5px"}}>
        <h4 style={{ margin: "0 0 10px", borderBottom: "1px solid #e2e8f0", paddingBottom: "5px" }}>Müşteri Genel Borç Durumları</h4>
  ...
  const renderSut = () => (
    <div className="tab-fade-in main-content-area">
      <button 
        ...
        className="btn-anim m-btn blue-btn"
      >
        ➕ YENİ SÜT GİRİŞİ
      </button>
      
      <div className="compact-totals">
        <div className="c-kutu" style={{ borderLeftColor: temaRengi }}>
          <span>SÜT</span>
          <b style={{ color: temaRengi }}>{fSayi(tSutKg)} KG</b>
        </div>
        <div className="c-kutu" style={{ borderLeftColor: temaRengi }}>
          <span>T. TUTAR</span>
          <b style={{ color: temaRengi }}>{fSayiNoDec(tSutTl)} ₺</b>
        </div>
      </div>
              ...
              <tr key={s.id}>
                ...
                <td className="actions-cell">
                  <div className="action-buttons">
                    ...
                    <button 
                      onClick={() => handleSoftDelete("sut_giris", s.id as string, "sut")} 
                      className="dl-btn"
                    >
                      ✕
                    </button>
  ...
  const renderSatis = () => (
    <div className="tab-fade-in main-content-area">
      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
         <button 
           onClick={handleYeniFisAc} 
           className="btn-anim m-btn green-btn" 
           style={{ margin: 0, flex: 2, fontSize: '13px' }}
         >
           ➕ YENİ SATIŞ FİŞİ
         </button>
         <button 
           ...
           className="btn-anim m-btn blue-btn" 
           style={{ margin: 0, flex: 1.2, fontSize: '13px', background: '#3b82f6' }}
         >
           💸 TAHSİLAT
         </button>
      </div>
      ...
      {/* KİŞİ FİLTRESİ */}
      <div style={{ display: 'flex', background: '#e2e8f0', borderRadius: '6px', overflow: 'hidden', marginBottom: '12px' }}>
        <button 
          onClick={() => setSatisFiltreKisi('benim')} 
          style={{ 
            flex: 1, padding: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', 
            background: satisFiltreKisi==='benim' ? '#2563eb' : 'transparent', 
            color: satisFiltreKisi==='benim' ? '#fff' : '#475569' 
          }}
        >
          Benim
        </button>
        <button 
          onClick={() => setSatisFiltreKisi('herkes')} 
          style={{ 
            flex: 1, padding: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', 
            background: satisFiltreKisi==='herkes' ? '#2563eb' : 'transparent', 
            color: satisFiltreKisi==='herkes' ? '#fff' : '#475569' 
          }}
        >
          Herkes
        </button>
      ...
      <div className="compact-totals">
        <div className="c-kutu" style={{ borderLeftColor: "#059669" }}>
          <span>TOPLAM</span>
          <b style={{ color: "#059669" }}>{fSayi(tFisToplam)} ₺</b>
        </div>
        <div className="c-kutu" style={{ borderLeftColor: "#2563eb" }}>
          <span>TAHSİLAT</span>
          <b style={{ color: "#2563eb" }}>{fSayi(tFisTahsilat)} ₺</b>
        </div>
        <div className="c-kutu" style={{ borderLeftColor: "#dc2626" }}>
          <span>AÇIK HESAP</span>
          <b style={{ color: "#dc2626" }}>{fSayi(tFisKalan)} ₺</b>
        </div>
      </div>
        ...
        <table className="tbl tbl-satis">
          <thead>
            <tr>
              ...
              <Th label="GENEL BORÇ" align="right" />
              <th></th>
            </tr>
          </thead>
          <tbody>
            {fFisList.map(f => {
              // Bayinin genel borcu hesaplanıp KALAN yerine yazılıyor
              const musteriBorcu = f.bayi === "SİSTEM İŞLEMİ" ? 0 : (bayiBorclari.find(b => b.isim === f.bayi)?.borc || 0);

              return (
                <tr key={f.id}>
                  <td>{f.tarih.split("-").reverse().slice(0, 2).join(".")}</td>
                  <td style={{ fontWeight: "bold", color: f.toplam_tutar === 0 ? "#8b5cf6" : "inherit" }} className="truncate-text-td">
                    {f.bayi}
                  </td>
                  <td style={{ textAlign: "right", color: "#059669", fontWeight: "bold" }}>
                    {f.toplam_tutar === 0 ? "-" : fSayi(f.toplam_tutar)}
                  </td>
                  <td style={{ textAlign: "right", color: "#2563eb", fontWeight: "bold" }}>
                    {fSayi(f.tahsilat)}
                  </td>
                  <td style={{ textAlign: "right", color: musteriBorcu > 0 ? "#dc2626" : (musteriBorcu < 0 ? "#059669" : "#64748b"), fontWeight: "bold" }}>
                    {musteriBorcu === 0 ? "-" : fSayi(musteriBorcu)}
                  </td>
                  <td className="actions-cell">
                    <div className="action-buttons">
                      <button 
                        onClick={() => handleFisDetayGoster(f)} 
                        className="btn-anim" 
                        style={{background: "none", border: "none", padding: "0 2px", cursor: "pointer", fontSize: "16px"}}
                      >
                        🔍
                      </button>
                      <button onClick={() => handleFisDuzenle(f)} className="ed-btn">
                        ✏️
                      </button>
                      <button onClick={() => handleSoftDelete("satis_fisleri", f.id as string, "satis")} className="dl-btn">
                        ✕
                      </button>
                    </div>
                  </td>
  ...
  const renderAnaliz = () => (
    <div className="tab-fade-in main-content-area">
      <div className="compact-totals" style={{marginTop: "5px"}}>
        <div className="c-kutu" style={{ borderLeftColor: "#8b5cf6" }}>
          <span>TOPLAM ADET/KG</span>
          <b style={{ color: "#8b5cf6" }}>{fSayi(tAnalizKg)}</b>
        </div>
        <div className="c-kutu" style={{ borderLeftColor: "#8b5cf6" }}>
          <span>TOPLAM TUTAR</span>
          <b style={{ color: "#8b5cf6" }}>{fSayi(tAnalizTutar)} ₺</b>
        </div>
      </div>
    ...
    <div className="tab-fade-in main-content-area">
      <div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "10px" }}>
        <button 
          ...
          className="btn-anim m-btn" 
          style={{background: "#dc2626", margin: 0, flex: 1, padding: "8px", fontSize: "13px", height: "36px"}}
        >
          ➕ YENİ GİDER
        </button>
      ...
      <div className="table-wrapper">
        <table className="tbl" style={{borderTop: "3px solid #fca5a5"}}>
          <thead>
            <tr>
              ...
              <tr key={g.id}>
                ...
                <td className="actions-cell">
                   <div className="action-buttons">
                     <button 
                       onClick={() => { 
                         setEditingGiderId(g.id); 
                         setGiderForm(g as any); 
                         setIsGiderModalOpen(true); 
                       }} 
                       className="ed-btn"
                     >
                     ...
                     <button onClick={() => handleSoftDelete("giderler", g.id as string, "gider")} className="dl-btn">
                       ✕
                     </button>
                   </div>
                </td>
  ...
  const renderUretim = () => (
    <div className="tab-fade-in main-content-area">
      <button 
        onClick={() => { 
          setUretimForm({ 
            tarih: bugun, cig_sut: "", sut_fiyat: uretimSonFiyatlar.sut, 
            sut_tozu: "", sut_tozu_fiyat: uretimSonFiyatlar.toz, 
            tereyag: "", tereyag_fiyat: uretimSonFiyatlar.yag, 
            katki_kg: "", katki_fiyat: uretimSonFiyatlar.katki, 
            su: "", kova_3_adet: "", kova_3_fiyat: uretimSonFiyatlar.kova3, 
            kova_5_adet: "", kova_5_fiyat: uretimSonFiyatlar.kova5, 
            cikti_3kg: "", satis_3_fiyat: uretimSonFiyatlar.satis3, 
      ...
      <div className="table-wrapper">
        <table className="tbl" style={{borderTop: "3px solid #c4b5fd"}}>
          <thead>
            <tr>
          ...
          <tbody>
            {sortData(periodUretim, uretimSort).map(u => (
              <tr key={u.id}>
                ...
                <td className="actions-cell">
                  <div className="action-buttons">
                    <button 
                      onClick={() => { 
                        setEditingUretimId(u.id); 
                        setUretimForm(u as any); 
                        setIsUretimModalOpen(true); 
                      }} 
                      className="ed-btn"
                    >
                    ...
                    <button onClick={() => handleSoftDelete("uretim", u.id as string, "uretim")} className="dl-btn">
                      ✕
                    </button>
                  </div>
                </td>
  ...
  const renderAyarlar = () => (
    <div className="tab-fade-in main-content-area" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      {/* MEVCUT LİSTE AYARLARI */}
      {[
        { 
          t: "ciftlikler", 
          title: "🥛 Süt Tedarikçileri", 
          open: acikTedarikci, 
          setOpen: setAcikTedarikci, 
          val: yeniTedarikci, 
          setVal: setYeniTedarikci, 
          data: tedarikciler, 
          btnColor: temaRengi, 
          isUrun: false 
        }, 
        { 
          t: "bayiler", 
          title: "🏢 Satış Bayileri / Müşteriler", 
          open: acikBayi, 
          setOpen: setAcikBayi, 
          val: yeniBayi, 
          setVal: setYeniBayi, 
            ...
            <div style={{ marginTop: "15px", borderTop: "1px solid #e2e8f0", paddingTop: "15px" }}>
              <div className="responsive-form" style={{marginBottom: "10px"}}>
                <input 
                  ...
                  value={sec.val} 
                  onChange={e => sec.setVal(e.target.value)} 
                ...
                {sec.isUrun && (
                  <input 
                    ...
                    type="number" 
                    value={yeniUrunFiyat} 
                    onChange={e => setYeniUrunFiyat(e.target.value)} 
                    className="m-inp num-inp" 
                  />
                )}
                
                <button 
                  onClick={() => ayarIslem(sec.t, sec.val, "ekle", null, sec.setVal)} 
                  className="p-btn btn-anim" 
                  style={{ background: sec.btnColor, flex: "0 0 100px" }}
                >
                  EKLE
                </button>
              </div>
              ...
              <div className="responsive-form" style={{marginTop: "5px"}}>
                <select 
                  ...
                  onChange={(e) => { 
                    ...
                    if(confirm(`"${targetItem?.isim}" kalıcı olarak silinecek, emin misiniz?`)) { 
                ...
                >
                  ...
                  {sec.data.map(d => (
                    <option key={d.id} value={d.id} style={{color: "#1e293b"}}>
                      {d.isim} {sec.isUrun && (d as Urun).fiyat ? `(${fSayi((d as Urun).fiyat)})` : ""}
                    </option>
                  ))}
                </select>
              </div>
      ...
      {/* YENİ ÖZELLİK: ÇÖP KUTUSU */}
      <div className="card" style={{ borderLeft: "5px solid #dc2626" }}>
        <div 
          onClick={() => setAcikCopKutusu(!acikCopKutusu)} 
          style={{ display: "flex", justifyContent: "space-between", cursor: "pointer" }}
        >
          <h4 style={{ margin: 0, color: "#dc2626" }}>🗑️ Çöp Kutusu (Silinenler)</h4>
          <span style={{ fontSize: "11px", color: "#64748b" }}>
            {acikCopKutusu ? "▲ GİZLE" : "▼ AÇ"}
          </span>
        </div>
        
        {acikCopKutusu && (
          <div style={{ marginTop: "15px", borderTop: "1px solid #e2e8f0", paddingTop: "15px", textAlign: "center" }}>
            <p style={{fontSize: "13px", color: "#64748b", margin: "0 0 10px 0"}}>
              Yanlışlıkla sildiğiniz fişler, süt veya gider kayıtları veritabanında güvende tutulur. 
              (Geri yükleme arayüzü Supabase üzerinden veya ileriki güncellemelerde eklenecektir.)
            </p>
            <button className="btn-anim" style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", padding: "8px 16px", borderRadius: "6px", fontSize: "12px", fontWeight: "bold" }}>
              SİLİNENLER GÜVENDE
            </button>
          </div>
        )}
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f8fafc", padding: "20px", boxSizing: "border-box" }}>
        <form 
          onSubmit={async (e) => { 
            ...
            if (target.elements.remember.checked) {
        ...
        >
          ...
          <input 
            ...
            style={{ width: "100%", marginBottom: "16px", padding: "8px", border: "1px solid #cbd5e1", borderRadius: "6px", boxSizing: "border-box" }} 
          />
          ...
          <label style={{ display: "flex", gap: "8px", fontSize: "13px", color: "#64748b", cursor: "pointer", marginBottom: "20px" }}>
            <input type="checkbox" id="remember" defaultChecked={!!localStorage.getItem('user')} /> 
            Beni Hatırla
          </label>
          
          <button 
            type="submit" 
            style={{ width: "100%", padding: "12px", background: temaRengi, color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}
          >
            Giriş Yap
          </button>
        </form>
  ...
  // --- ANA UYGULAMA RENDER ALANI ---
  return (
    <div className="app-container" style={{ fontSize: `${fontSize}px` }}>
      
      {/* ÜST MENÜ (HEADER) - KULLANICI ADI VE DÖNEM EKLENDİ */}
      <header className="header-style">
        <b style={{ color: temaRengi, fontSize: "16px", marginLeft: "10px" }}>SULTANKÖY V4</b>
        ...
        <div style={{display: 'flex', gap: '15px', alignItems: 'center', marginRight: '10px'}}>
           <select 
              ...
              className="m-inp" 
              style={{padding: "2px 8px", height: "28px", fontSize: "12px", borderRadius: "6px", fontWeight: "bold", cursor: "pointer", background: "#f1f5f9", borderColor: "#cbd5e1"}}
           >
             {aylar.map(ay => (
               <option key={ay} value={ay}>{ay.replace('-', ' / ')}</option>
             ))}
             <option value="KAPAT">⚠️ Dönemi Kapat</option>
           </select>
           
           {/* KULLANICI ADI (İsim) */}
           <span style={{fontSize: "13px", fontWeight: "bold", color: "#0f172a", background: "#f1f5f9", padding: "4px 8px", borderRadius: "6px"}}>
             {username}
           </span>
           
           <button 
             onClick={() => supabase.auth.signOut()} 
             style={{ background: "none", border: "1px solid #fecaca", borderRadius: "50%", width: "32px", height: "32px", color: "#dc2626", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}
             title="Çıkış Yap"
           >
      ...
      <main className="main-content">
        ...
        {/* DÖNEM KAPATMA ONAY MODALI */}
        {isDonemModalOpen && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 1500, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
            <div style={{ backgroundColor: "#fff", padding: "20px", borderRadius: "12px", width: "100%", maxWidth: "320px", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.2)" }}>
              <h3 style={{marginTop: 0, color: "#dc2626", borderBottom: "1px solid #e2e8f0", paddingBottom: "10px"}}>⚠️ Dönem Kapatma</h3>
              <p style={{fontSize: "13px", color: "#475569", lineHeight: "1.5"}}>
                <b>{aktifDonem}</b> dönemini kapatmak üzeresiniz.<br/><br/>
                {['satis', 'analiz'].includes(activeTab) 
                  ? "Açık hesabı olan tüm müşterilerin borçları yeni aya 'Devir' olarak otomatik aktarılacaktır." 
                  : "Bu sekmenin dönemi yeni aya geçirilecektir."}
                <br/><br/>Emin misiniz?
              </p>
              <div style={{display: "flex", gap: "10px", marginTop: "20px"}}>
                <button onClick={() => setIsDonemModalOpen(false)} style={{flex: 1, padding: "10px", background: "#f1f5f9", color: "#475569", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer"}}>İPTAL</button>
                <button onClick={() => { handleDonemKapat(); setIsDonemModalOpen(false); }} style={{flex: 1, padding: "10px", background: "#dc2626", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer"}}>KAPAT VE DEVRET</button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeFilterModal && (
          ...
          >
            <div 
              style={{ backgroundColor: "#fff", padding: "15px", borderRadius: "10px", width: "100%", maxWidth: "260px", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.2)" }} 
              ...
              {activeFilterModal.endsWith('_tarih') && (
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  <div>
                    ...
                    <input 
                      type="date" 
                      value={activeFilterModal.includes('sut') ? sutFiltre.baslangic : activeFilterModal.includes('fis') ? fisFiltre.baslangic : analizFiltre.baslangic} 
                      onChange={(e) => { 
                        if(activeFilterModal.includes('sut')) setSutFiltre({...sutFiltre, baslangic: e.target.value}); 
                        if(activeFilterModal.includes('fis')) setFisFiltre({...fisFiltre, baslangic: e.target.value}); 
                        if(activeFilterModal.includes('analiz')) setAnalizFiltre({...analizFiltre, baslangic: e.target.value}); 
                      }} 
                      className="m-inp" 
                  ...
                  <div>
                    ...
                    <input 
                      type="date" 
                      value={activeFilterModal.includes('sut') ? sutFiltre.bitis : activeFilterModal.includes('fis') ? fisFiltre.bitis : analizFiltre.bitis} 
                      onChange={(e) => { 
                        if(activeFilterModal.includes('sut')) setSutFiltre({...sutFiltre, bitis: e.target.value}); 
                        if(activeFilterModal.includes('fis')) setFisFiltre({...fisFiltre, bitis: e.target.value}); 
                        if(activeFilterModal.includes('analiz')) setAnalizFiltre({...analizFiltre, bitis: e.target.value}); 
                      }} 
                      className="m-inp" 
                      style={{width: "100%", marginTop: "4px"}} 
                    />
              ...
              <div style={{ maxHeight: "250px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "10px", padding: "4px 0" }}>
                {activeFilterModal === 'sut_ciftlik' && tedarikciler.map(t => (
                  <label key={t.id} style={{display: "flex", alignItems: "center", gap: "8px", fontSize: "14px"}}>
                    <input 
                      type="checkbox" 
                      checked={sutFiltre.ciftlikler.includes(t.isim)} 
                      onChange={() => handleCheckboxToggle('ciftlikler', setSutFiltre, t.isim)} 
                ...
                {activeFilterModal.includes('_bayi') && bayiler.map(b => (
                  <label key={b.id} style={{display: "flex", alignItems: "center", gap: "8px", fontSize: "14px"}}>
                    <input 
                      type="checkbox" 
                      checked={activeFilterModal === 'fis_bayi' ? fisFiltre.bayiler.includes(b.isim) : analizFiltre.bayiler.includes(b.isim)} 
                      onChange={() => handleCheckboxToggle('bayiler', activeFilterModal === 'fis_bayi' ? setFisFiltre : setAnalizFiltre, b.isim)} 
                      style={{width:"18px", height:"18px"}}
                    /> 
                    {b.isim}
                  </label>
                ...
                {activeFilterModal === 'analiz_urun' && urunler.map(u => (
                  <label key={u.id} style={{display: "flex", alignItems: "center", gap: "8px", fontSize: "14px"}}>
                    <input 
                      type="checkbox" 
                      checked={analizFiltre.urunler.includes(u.isim)} 
                      onChange={() => handleCheckboxToggle('urunler', setAnalizFiltre, u.isim)} 
                      style={{width:"18px", height:"18px"}}
                    /> 
                    {u.isim}
                  </label>
              ...
              <div style={{display: "flex", gap: "8px", marginTop: "15px"}}>
                <button 
                  onClick={() => { 
                    ...
                    if(activeFilterModal.includes('_tarih')){ 
                      setSutFiltre({...sutFiltre, baslangic: '', bitis: ''}); 
                      setFisFiltre({...fisFiltre, baslangic: '', bitis: ''}); 
                      setAnalizFiltre({...analizFiltre, baslangic: '', bitis: ''}); 
                    } 
                  }} 
                ...
                <button 
                  onClick={() => setActiveFilterModal(null)} 
                  style={{flex: 1, padding: "10px", background: activeFilterModal.includes('analiz') ? '#8b5cf6' : temaRengi, color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold"}}
                >
                  UYGULA
                </button>
              </div>
        ...
        {isFisModalOpen && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1200, padding: "8px" }}>
            <div style={{ backgroundColor: "#fff", width: "95vw", maxWidth: "450px", maxHeight: "95vh", borderRadius: "8px", display: "flex", flexDirection: "column", animation: "fadeIn 0.2s ease-out", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
              ...
              <div style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", background: editingFisId ? "#fef3c7" : "#f8fafc", borderRadius: "8px 8px 0 0" }}>
                <h3 style={{ margin: "0", color: editingFisId ? "#b45309" : "#059669", fontSize: "15px" }}>
                  {editingFisId ? "✏️ Fişi Düzenle" : "🧾 Yeni Satış Fişi"}
                </h3>
              ...
              <div style={{ padding: "10px 12px", overflowY: "auto", flex: 1 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '8px' }}>
                  {urunler.map(u => {
                    ...
                    // SATIR TUTARI HESAPLAMA (Adet/Kg x Fiyat)
                    const aNum = Number(fisDetay[u.id]?.adet) || 0;
                    const kNum = Number(fisDetay[u.id]?.kg) || 0;
                    const fNum = Number(fisDetay[u.id]?.fiyat) || 0;
                    const isK = u.isim.match(/(3|4|5)\s*kg/i);
                    const miktarNum = isK ? aNum : (kNum > 0 ? kNum : aNum);
                    const satirTutar = miktarNum * fNum;
                    ...
                    return (
                      <div 
                        ...
                        style={{ 
                          ...
                          alignItems: 'center', 
                          padding: '4px 6px', 
                          background: isFilled ? (editingFisId ? '#fef3c7' : '#ecfdf5') : '#f8fafc', 
                          borderRadius: '4px', 
                          border: isFilled ? (editingFisId ? '1px solid #fde68a' : '1px solid #a7f3d0') : '1px solid #e2e8f0' 
                        }}
                      >
                        <div style={{ flex: 1, minWidth: "85px", fontWeight: 'bold', fontSize: "12px", color: isFilled ? (editingFisId ? "#b45309" : "#065f46") : "#475569", whiteSpace: "normal", lineHeight: "1.2" }}>
                          {u.isim}
                        </div>
                        <input 
                          placeholder="Adet" 
                          type="number" 
                          value={fisDetay[u.id]?.adet || ""} 
                          onChange={handleAdetChange} 
                          className="m-inp" 
                          style={{flex: "0 0 45px", width: "45px", padding: "4px 2px", textAlign: "center", background: isFilled ? "#fff" : "", fontSize: "12px", height:"24px"}} 
                        />
                        <input 
                          placeholder="KG" 
                          type="number" 
                          step="0.01" 
                          value={fisDetay[u.id]?.kg || ""} 
                          onChange={e => setFisDetay({...fisDetay, [u.id]: {...fisDetay[u.id], kg: e.target.value}})} 
                          className="m-inp" 
                          style={{flex: "0 0 50px", width: "50px", padding: "4px 2px", textAlign: "center", background: isFilled ? "#fff" : "", fontSize: "12px", height:"24px"}} 
                        />
                        ...
                        <input 
                          ...
                          type="number" 
                          step="0.01" 
                          value={fisDetay[u.id]?.fiyat || ""} 
                          onChange={e => setFisDetay({...fisDetay, [u.id]: {...fisDetay[u.id], fiyat: e.target.value}})} 
                          className="m-inp" 
                          style={{flex: "0 0 60px", width: "60px", padding: "4px 2px", textAlign: "right", background: isFilled ? "#fff" : "", fontSize: "12px", height:"24px"}} 
                        />
                        {/* ANLIK SATIR TUTARI */}
                        <div style={{ flex: "0 0 50px", width: "50px", textAlign: "right", fontSize: "12px", fontWeight: "bold", color: "#059669" }}>
                           {satirTutar > 0 ? fSayi(satirTutar) : "-"}
                        </div>
                      </div>
                    );
                  })}
                  ...
                  {(gosterilenEkler.iade || Number(fisDetay["v_iade"]?.adet) > 0 || Number(fisDetay["v_iade"]?.kg) > 0) && (
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center', padding: '4px 6px', background: '#fef2f2', borderRadius: '4px', border: '1px solid #fecaca', marginTop: "4px" }}>
                      <div style={{ flex: 1, minWidth: "85px", fontWeight: 'bold', fontSize: "12px", color: "#dc2626", whiteSpace: "normal", lineHeight: "1.2" }}>
                        İade
                      </div>
                      <input 
                        placeholder="Adet" 
                        type="number" 
                        value={fisDetay["v_iade"]?.adet || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_iade: {...fisDetay["v_iade"], adet: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 45px", width: "45px", padding: "4px 2px", textAlign: "center", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      <input 
                        placeholder="KG" 
                        type="number" 
                        step="0.01" 
                        value={fisDetay["v_iade"]?.kg || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_iade: {...fisDetay["v_iade"], kg: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 50px", width: "50px", padding: "4px 2px", textAlign: "center", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      <div style={{fontSize:"12px", color:"#94a3b8", width:"8px", textAlign:"center"}}>{"x"}</div>
                      <input 
                        placeholder="Fiyat" 
                        type="number" 
                        step="0.01" 
                        value={fisDetay["v_iade"]?.fiyat || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_iade: {...fisDetay["v_iade"], fiyat: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 60px", width: "60px", padding: "4px 2px", textAlign: "right", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      {/* ANLIK İADE TUTARI */}
                      <div style={{ flex: "0 0 50px", width: "50px", textAlign: "right", fontSize: "12px", fontWeight: "bold", color: "#dc2626" }}>
                         {Number(fisDetay["v_iade"]?.adet || 0) > 0 || Number(fisDetay["v_iade"]?.kg || 0) > 0 ? fSayi((Number(fisDetay["v_iade"]?.kg) > 0 ? Number(fisDetay["v_iade"]?.kg) : Number(fisDetay["v_iade"]?.adet)) * Number(fisDetay["v_iade"]?.fiyat)) : "-"}
                      </div>
                    </div>
                  )}
                  ...
                  {(gosterilenEkler.bos_kova || Number(fisDetay["v_bos_kova"]?.adet) > 0 || Number(fisDetay["v_bos_kova"]?.kg) > 0) && (
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center', padding: '4px 6px', background: '#fef2f2', borderRadius: '4px', border: '1px solid #fecaca', marginTop: "4px" }}>
                      ...
                      <input 
                        placeholder="Adet" 
                        type="number" 
                        value={fisDetay["v_bos_kova"]?.adet || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_bos_kova: {...fisDetay["v_bos_kova"], adet: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 45px", width: "45px", padding: "4px 2px", textAlign: "center", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      <input 
                        ...
                        type="number" 
                        step="0.01" 
                        value={fisDetay["v_bos_kova"]?.kg || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_bos_kova: {...fisDetay["v_bos_kova"], kg: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 50px", width: "50px", padding: "4px 2px", textAlign: "center", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      <div style={{fontSize:"12px", color:"#94a3b8", width:"8px", textAlign:"center"}}>{"x"}</div>
                      <input 
                        ...
                        type="number" 
                        step="0.01" 
                        value={fisDetay["v_bos_kova"]?.fiyat || ""} 
                        onChange={e => setFisDetay({...fisDetay, v_bos_kova: {...fisDetay["v_bos_kova"], fiyat: e.target.value}})} 
                        className="m-inp" 
                        style={{flex: "0 0 60px", width: "60px", padding: "4px 2px", textAlign: "right", background: "#fff", fontSize: "12px", height:"24px", borderColor: "#fca5a5"}} 
                      />
                      {/* ANLIK KOVA TUTARI */}
                      <div style={{ flex: "0 0 50px", width: "50px", textAlign: "right", fontSize: "12px", fontWeight: "bold", color: "#dc2626" }}>
                         {Number(fisDetay["v_bos_kova"]?.adet || 0) > 0 || Number(fisDetay["v_bos_kova"]?.kg || 0) > 0 ? fSayi((Number(fisDetay["v_bos_kova"]?.kg) > 0 ? Number(fisDetay["v_bos_kova"]?.kg) : Number(fisDetay["v_bos_kova"]?.adet)) * Number(fisDetay["v_bos_kova"]?.fiyat)) : "-"}
                      </div>
                    </div>
                  )}
                ...
                <div style={{display: "flex", gap: "6px"}}>
                  <select 
                    value={fisUst.odeme_turu} 
                    onChange={e => setFisUst({ ...fisUst, odeme_turu: e.target.value })} 
                    className="m-inp" 
                    style={{flex: "0 0 95px", padding: "6px 4px", fontSize: "12px", height: "30px"}}
                  >
                    <option value="PEŞİN">💵 PEŞİN</option>
                    <option value="VADE">⏳ VADE</option>
                    <option value="KREDİ KARTI">💳 K.KARTI</option>
                    <option value="HAVALE/EFT">🏦 HAVALE</option>
                  </select>
                  
                  <input 
                    placeholder="Açıklama/Not..." 
                    value={fisUst.aciklama} 
                    onChange={e => setFisUst({ ...fisUst, aciklama: e.target.value })} 
                    className="m-inp grow-inp" 
                ...
                <div style={{display: "flex", gap: "6px", marginTop: "6px"}}>
                  <input 
                    placeholder="Teslim Alan (İsim Soyisim)" 
                    value={fisUst.teslim_alan || ""} 
                    onChange={e => setFisUst({ ...fisUst, teslim_alan: e.target.value })} 
                    className="m-inp grow-inp" 
                    style={{padding: "6px 8px", fontSize: "12px", height: "30px"}} 
                  />
                </div>
              ...
              <div style={{ padding: "10px 12px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", borderRadius: "0 0 8px 8px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                  <span style={{color: "#0f172a", fontWeight: "bold", fontSize: "14px"}}>Genel Toplam:</span>
                  <b style={{color: "#0f172a", fontSize: "16px"}}>{fSayi(fisCanliToplam)} ₺</b>
                </div>
                
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                  <span style={{color: "#2563eb", fontWeight: "bold", fontSize: "13px"}}>Tahsil Edilen:</span>
                  <input 
                    type="number" 
                    placeholder="Alınan..." 
                    value={fisUst.tahsilat} 
                    onChange={e => setFisUst({ ...fisUst, tahsilat: e.target.value })} 
                    className="m-inp" 
                    style={{ flex: "0 0 90px", padding: "4px 6px", textAlign: "right", borderColor: "#bfdbfe", fontSize: "13px", height: "28px" }} 
                  />
                </div>
                
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px", borderTop: "1px dashed #cbd5e1", paddingTop: "6px" }}>
                  <span style={{color: (fisCanliToplam - Number(fisUst.tahsilat || 0)) > 0 ? "#dc2626" : "#059669", fontWeight: "bold", fontSize: "13px"}}>
                    BU FİŞTEN KALAN:
                  </span>
                  <b style={{color: (fisCanliToplam - Number(fisUst.tahsilat || 0)) > 0 ? "#dc2626" : "#059669", fontSize: "14px"}}>
                    {fSayi(fisCanliToplam - Number(fisUst.tahsilat || 0))} ₺
                  </b>
                </div>
                ...
                {aktifBayi && (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                      <span style={{color: "#64748b", fontSize: "11px"}}>Önceki Bakiye (Genel):</span>
                      <b style={{color: "#64748b", fontSize: "12px"}}>{fSayi(eskiBorc)} ₺</b>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", background: "#fef2f2", padding: "6px", borderRadius: "6px", border: "1px solid #fecaca" }}>
                      <span style={{color: "#dc2626", fontWeight: "bold", fontSize: "12px"}}>GENEL TOPLAM BORÇ:</span>
                      <b style={{color: "#dc2626", fontSize: "16px"}}>{fSayi(toplamGenelBorc)} ₺</b>
                    </div>
                  </>
                )}
                ...
                <button 
                  ...
                  className="p-btn btn-anim" 
                  style={{ background: editingFisId ? "#f59e0b" : "#059669", width: "100%", height: "40px", fontSize: "14px" }}
                >
                  {editingFisId ? "DEĞİŞİKLİKLERİ KAYDET" : "FİŞİ KAYDET"}
                </button>
        ...
        {/* MÜŞTERİ ÇIKTI (FİŞ DETAY) EKRANI */}
        {sonFisData && (
            ...
            <div style={{ backgroundColor: "#f8fafc", borderRadius: "10px", width: "95vw", maxWidth: "340px", overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "95vh" }}>
              ...
              <div style={{ overflowY: "auto", flex: 1 }}>
                <div id="print-receipt" style={{ background: "#fff", padding: "15px", textAlign: "center", borderBottom: "1px dashed #cbd5e1" }}>
                  <h2 style={{ margin: "0 0 2px", color: "#000", fontSize: "18px" }}>SULTANKÖY</h2>
                  <div style={{ color: "#000", fontSize: "11px", marginBottom: "12px" }}>Süt Ürünleri</div>
                  
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#000", marginBottom: "2px" }}>
                    <span>Tarih:</span>
                    <b>{sonFisData.tarih.split("-").reverse().join(".")}</b>
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#000", marginBottom: "2px" }}>
                    <span>Sayın:</span>
                    <b style={{textAlign: "right"}}>{sonFisData.bayi}</b>
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "9px", color: "#000", marginBottom: "12px" }}>
                    <span>Fiş No:</span>
                    <b>{sonFisData.fis_no}</b>
                  </div>
                    ...
                    <table style={{ width: '100%', fontSize: '11px', textAlign: 'left', borderCollapse: 'collapse', marginBottom: '12px', color: '#000' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid #000' }}>
                          <th style={{ paddingBottom: '4px' }}>Ürün</th>
                          <th style={{ paddingBottom: '4px', textAlign: 'center' }}>Adet</th>
                          <th style={{ paddingBottom: '4px', textAlign: 'center' }}>KG</th>
                          <th style={{ paddingBottom: '4px', textAlign: 'right' }}>B.Fiyat</th>
                          <th style={{ paddingBottom: '4px', textAlign: 'right' }}>Tutar</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", paddingTop: "6px", color: "#000", borderTop: "1px dashed #ccc", marginTop: "4px" }}>
                    <span>Genel Toplam:</span>
                    <b>{fSayi(sonFisData.genelToplam)} ₺</b>
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", paddingTop: "4px", color: "#000" }}>
                    <span>Tahsil Edilen:</span>
                    <b>{fSayi(sonFisData.tahsilat)}</b>
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", paddingTop: "4px", color: "#000" }}>
                    <span>Bu Fiş Kalan:</span>
                    <b>{fSayi(sonFisData.kalanBakiye)}</b>
                  </div>
                  ...
                  {sonFisData.gosterBakiye && (sonFisData.eskiBorc !== 0 || sonFisData.genelBorc !== 0) && (
                    <div style={{ marginTop: "6px", borderTop: "1px solid #000", paddingTop: "6px" }}>
                      {sonFisData.eskiBorc !== 0 && (
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", color: "#000", marginBottom: "2px" }}>
                          <span>Önceki Bakiye (Genel):</span>
                          <b>{fSayi(sonFisData.eskiBorc)} ₺</b>
                        </div>
                      )}
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", color: "#000", fontWeight: "bold" }}>
                        <span>TOPLAM BORÇ:</span>
                        <b>{fSayi(sonFisData.genelBorc)} ₺</b>
                      </div>
                    </div>
                  )}
                  ...
                  {sonFisData.teslim_alan && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#000", marginTop: "10px" }}>
                      ...
                      <b style={{textAlign: "right"}}>{sonFisData.teslim_alan}</b>
                    </div>
                  )}
                  
                  <div style={{ textAlign: "right", fontSize: "10px", color: "#000", marginTop: "10px", borderTop: "1px dashed #ccc", paddingTop: "4px" }}>
                    Ödeme: {sonFisData.odeme}
                  </div>
                  
                  <div style={{ textAlign: "center", fontSize: "9px", color: "#000", marginTop: "12px" }}>
              ...
              <div className="no-print" style={{ padding: "10px", display: "flex", flexDirection: "column", gap: "6px", background: "#f8fafc", borderTop: "1px solid #cbd5e1" }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', background: '#e2e8f0', padding: '8px', borderRadius: '6px', cursor: 'pointer', color: '#1e293b', fontWeight: 'bold' }}>
                  <input 
                    type="checkbox" 
                    checked={sonFisData.gosterBakiye} 
                    onChange={e => setSonFisData({...sonFisData, gosterBakiye: e.target.checked})} 
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  <button 
                    ...
                    className="btn-anim" 
                    style={{ flex: 1, padding: "10px", background: "#475569", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", fontSize: "12px", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                  >
                    <span style={{fontSize:'16px'}}>🖨️</span> YAZDIR
                  </button>
                  <button 
                    onClick={handleWhatsappResimGonder} 
                    className="btn-anim" 
                    style={{ flex: 1, padding: "10px", background: "#25D366", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", fontSize: "13px", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                  >
                    WHATSAPP
                  </button>
                </div>
                <div style={{ display: "flex", gap: "6px", marginTop: "4px" }}>
                  <button 
                    onClick={() => {
                      const fakeFis = { 
                        id: sonFisData.id, 
                        fis_no: sonFisData.fis_no, 
                        tarih: sonFisData.tarih, 
                        bayi: sonFisData.bayi, 
                        odeme_turu: sonFisData.odeme, 
                        aciklama: sonFisData.aciklama || "", 
                        tahsilat: sonFisData.tahsilat, 
                        kalan_bakiye: sonFisData.kalanBakiye, 
                        toplam_tutar: sonFisData.genelToplam 
                      };
                      setSonFisData(null);
                  ...
                  <button 
                    onClick={() => {
                      handleSoftDelete("satis_fisleri", sonFisData.id, "satis");
                      setSonFisData(null);
                    }} 
                    className="btn-anim" 
                ...
                <button 
                  onClick={() => setSonFisData(null)} 
                  className="btn-anim" 
                  style={{ width: "100%", padding: "8px", background: "transparent", color: "#64748b", border: "1px solid #cbd5e1", borderRadius: "8px", fontWeight: "bold", fontSize: "11px", marginTop: "2px" }}
                >
                  KAPAT
                </button>
              </div>
            ...
            <div style={{ backgroundColor: "#fff", width: "95vw", maxWidth: "350px", borderRadius: "12px", display: "flex", flexDirection: "column", animation: "fadeIn 0.2s ease-out", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
               ...
               <div style={{ padding: "15px", display: "flex", flexDirection: "column", gap: "10px" }}>
                 <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    />
                    <input 
                      list="bayiler-tahsilat" 
                      placeholder="Müşteri/Bayi Seç..." 
                      value={tahsilatForm.bayi} 
                      onChange={e => setTahsilatForm({ ...tahsilatForm, bayi: e.target.value })} 
                      className="m-inp" 
                 ...
                 <div style={{ display: "flex", gap: "8px" }}>
                    <div style={{flex: 1}}>
                      <label style={{fontSize: "11px", color: "#64748b"}}>Tahsilat Tutarı (₺)</label>
                      <input 
                        type="number" 
                        step="0.01" 
                        value={tahsilatForm.miktar} 
                        onChange={e => setTahsilatForm({ ...tahsilatForm, miktar: e.target.value })} 
                        className="m-inp" 
                        style={{width: "100%", textAlign: "right", color: "#059669", fontWeight: "bold"}} 
                      />
                    </div>
                    <div style={{flex: 1}}>
                      ...
                      <select 
                        ...
                        onChange={e => setTahsilatForm({ ...tahsilatForm, odeme_turu: e.target.value })} 
                        className="m-inp" 
                        style={{width: "100%"}}
                      >
                        <option value="PEŞİN">💵 PEŞİN</option>
               ...
               <div style={{ padding: "12px 15px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", borderRadius: "0 0 12px 12px" }}>
                 <button 
                   ...
                   className="p-btn btn-anim" 
                   style={{ background: "#2563eb", width: "100%", height: "45px", fontSize: "15px" }}
                 >
                   KAYDET
                 </button>
               </div>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1200, padding: "10px" }}>
            <div style={{ backgroundColor: "#fff", width: "95vw", maxWidth: "350px", borderRadius: "12px", display: "flex", flexDirection: "column", animation: "fadeIn 0.2s ease-out", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
              <div style={{ padding: "12px 15px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", background: editingGiderId ? "#fef2f2" : "#f8fafc", borderRadius: "12px 12px 0 0" }}>
                <h3 style={{ margin: "0", color: "#dc2626", fontSize: "15px" }}>{editingGiderId ? "✏️ Gider Düzenle" : "💸 Yeni Gider Kaydı"}</h3>
              ...
              <div style={{ padding: "15px", display: "flex", flexDirection: "column", gap: "10px" }}>
                <div style={{ display: "flex", gap: "8px" }}>
                   ...
                   <select 
                     value={giderForm.tur} 
                     onChange={e => setGiderForm({ ...giderForm, tur: e.target.value })} 
                     className="m-inp" 
                     style={{ flex: 2, fontWeight: "bold" }}
                   >
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="m-inp" 
                    style={{width: "100%", textAlign: "right", color: "#dc2626", fontWeight: "bold"}} 
                  />
                </div>
                <div>
                  <label style={{fontSize: "11px", color: "#64748b"}}>Açıklama / Not</label>
              ...
              <div style={{ padding: "12px 15px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", borderRadius: "0 0 12px 12px" }}>
                <button 
                  onClick={handleGiderKaydet} 
                  className="p-btn btn-anim" 
                  style={{ background: "#dc2626", width: "100%", height: "45px", fontSize: "15px" }}
                >
                  {editingGiderId ? "GÜNCELLE" : "KAYDET"}
                </button>
              </div>
            </div>
          </div>
              ...
              <div style={{ padding: "15px", display: "flex", flexDirection: "column", gap: "10px" }}>
                <div style={{ display: "flex", gap: "8px" }}>
                   <input 
                     ...
                     value={sutForm.tarih} 
                     onClick={handleDateClick}
                     onChange={e => setSutForm({ ...sutForm, tarih: e.target.value })} 
                     className="m-inp" 
                     style={{ flex: 1 }} 
                   />
                   <select 
                     value={sutForm.ciftlik} 
                     onChange={e => setSutForm({ ...sutForm, ciftlik: e.target.value })} 
                     className="m-inp" 
                     style={{ flex: 2, fontWeight: "bold" }}
                   >
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <div style={{flex: 1}}>
                    ...
                    <input 
                      type="number" 
                      value={sutForm.kg} 
                      onChange={e => setSutForm({ ...sutForm, kg: e.target.value })} 
                      className="m-inp" 
                  ...
                  <div style={{flex: 1}}>
                    ...
                    <input 
                      ...
                      step="0.01" 
                      value={sutForm.fiyat} 
                      onChange={e => setSutForm({ ...sutForm, fiyat: e.target.value })} 
                      className="m-inp" 
                      style={{width: "100%", textAlign: "right"}} 
                    />
                  </div>
                ...
                <div>
                  ...
                  <input 
                    placeholder="Opsiyonel..." 
                    value={sutForm.aciklama} 
                    onChange={e => setSutForm({ ...sutForm, aciklama: e.target.value })} 
                    className="m-inp" 
                    style={{width: "100%"}} 
                  />
                </div>
              ...
              <div style={{ padding: "12px 15px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", borderRadius: "0 0 12px 12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                  ...
                  <b style={{color: temaRengi, fontSize: "18px"}}>
                    {fSayi((Number(sutForm.kg) || 0) * (Number(sutForm.fiyat) || 0))} ₺
                  </b>
                </div>
                <button 
                  onClick={handleSutKaydet} 
                  className="p-btn btn-anim" 
                  style={{ background: editingSutId ? "#f59e0b" : temaRengi, width: "100%", height: "45px", fontSize: "15px" }}
                >
        ...
        {isUretimModalOpen && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1200, padding: "10px" }}>
            <div style={{ backgroundColor: "#fff", width: "95vw", maxWidth: "420px", borderRadius: "12px", display: "flex", flexDirection: "column", animation: "fadeIn 0.2s ease-out", maxHeight: "95vh" }}>
              ...
              <div style={{ padding: "10px", overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "8px" }}>
                <input 
                  type="date" 
                  value={uretimForm.tarih} 
                  onClick={handleDateClick}
                  onChange={e => setUretimForm({ ...uretimForm, tarih: e.target.value })} 
                ...
                <div>
                  <input 
                    placeholder="Açıklama/Not..." 
                    value={uretimForm.aciklama} 
                    onChange={e => setUretimForm({ ...uretimForm, aciklama: e.target.value })} 
                    className="m-inp small-inp" 
                    style={{width: "100%"}} 
                  />
                </div>
              ...
              <div style={{ padding: "10px 12px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", borderRadius: "0 0 12px 12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
                  ...
                  <b style={{color: "#dc2626", fontSize: "14px"}}>
                    {fSayi(
                      (Number(uretimForm.cig_sut||0) * Number(uretimForm.sut_fiyat||0)) +
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  ...
                  <b style={{color: "#059669", fontSize: "14px"}}>
                    {fSayi(
                      ((Number(uretimForm.cikti_3kg||0) * Number(uretimForm.satis_3_fiyat||0)) +
                ...
                <button 
                  onClick={handleUretimKaydet} 
                  className="p-btn btn-anim" 
                  style={{ background: "#8b5cf6", width: "100%", height: "40px", fontSize: "14px" }}
                >
                  {editingUretimId ? "GÜNCELLE" : "KAYDET"}
                </button>
              </div>
            </div>
          </div>
        ...
        {detayNot && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1500, padding: "20px" }} onClick={() => setDetayNot(null)}>
            <div style={{ backgroundColor: "#fff", padding: "25px", borderRadius: "16px", width: "100%", maxWidth: "350px", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }} onClick={e => e.stopPropagation()}>
              <h3 style={{ margin: "0 0 15px", borderBottom: "1px solid #e2e8f0", paddingBottom: "10px" }}>
                Açıklama / Not
              </h3>
              <p style={{ margin: "0 0 25px", color: "#475569", lineHeight: "1.6", wordWrap: "break-word" }}>
                {detayNot}
              </p>
              <button 
                onClick={() => setDetayNot(null)} 
                style={{ width: "100%", padding: "12px", background: "#f1f5f9", border: "1px solid #cbd5e1", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}
              >
                KAPAT
              </button>
            </div>
      ...
      <footer className="fixed-nav main-content-area">
        {[
          ...
          <button 
            ...
            className={`n-item btn-anim ${activeTab === item.id ? 'active' : ''}`} 
            style={
              activeTab === item.id 
                ? { 
                    color: item.id === 'analiz' ? '#8b5cf6' : item.id === 'gider' ? '#dc2626' : item.id === 'uretim' ? '#8b5cf6' : temaRengi, 
                    borderTopColor: item.id === 'analiz' ? '#8b5cf6' : item.id === 'gider' ? '#dc2626' : item.id === 'uretim' ? '#8b5cf6' : temaRengi 
                  } 
                : {}
            }
          >
            <span style={{ fontSize: "16px", marginBottom: "2px" }}>{item.i}</span>
            <span style={{ fontSize: "9px", fontWeight: "bold" }}>{item.id.toUpperCase()}</span>
          </button>
        ))}
      </footer>
