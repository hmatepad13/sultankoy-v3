...
<div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', paddingRight: '4px' }}>
    <h4 style={{ margin: "0 0 5px", fontSize: '13px', color: '#dc2626' }}>Son Silinen Kayıtlar</h4>
    {copKutusuList.map(c => (
        <div key={c.id} style={{ padding: '10px', background: '#fff', border: '1px solid #fecaca', borderRadius: '8px', fontSize: '12px', color: '#1e293b', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px", borderBottom: "1px dashed #fecaca", paddingBottom: "4px" }}>
                <strong style={{ color: '#dc2626' }}>{c.tablo_adi === 'satis_fisleri' ? 'SATIŞ FİŞİ' : c.tablo_adi.toUpperCase()}</strong>
                <span style={{ fontSize: '10px', color: '#94a3b8' }}>{c.silinme_tarihi ? new Date(c.silinme_tarihi).toLocaleString('tr-TR') : ''}</span>
            </div>
            
            {/* VERİ TÜRÜNE GÖRE OKUNAKLI GÖSTERİM */}
            <div style={{ lineHeight: '1.5' }}>
                {c.tablo_adi === 'satis_fisleri' && (
                    <>
                ...
                {['satis_fisleri', 'giderler', 'sut_giris'].indexOf(c.tablo_adi) === -1 && (
