...
export default function App() {
  ...
  const [sutFiltre, setSutFiltre] = useState<{
    ciftlikler: string[];
    baslangic: string;
    bitis: string;
  }>({ ciftlikler: [], baslangic: '', bitis: '' });
  const [sutSort, setSutSort] = useState<any>({
    key: 'tarih',
    direction: 'desc',
  });
  ...
  const [fisDetay, setFisDetay] = useState<
    Record<string, { adet: string; fiyat: string }>
  >({});
  const [sonFisData, setSonFisData] = useState<any>(null);

  const [fisFiltre, setFisFiltre] = useState<{
    bayiler: string[];
    baslangic: string;
    bitis: string;
  }>({ bayiler: [], baslangic: '', bitis: '' });
  const [fisSort, setFisSort] = useState<any>({
    key: 'tarih',
    direction: 'desc',
  });

  // --- ANALİZ STATE'LERİ (Çoklu Seçim İçin Array Yapıldı) ---
  ...
  const [analizSort, setAnalizSort] = useState<any>({
    key: 'tarih',
    direction: 'desc',
  });
  ...
  const [activeFilterModal, setActiveFilterModal] = useState<
    'sut_ciftlik' | 'fis_bayi' | 'analiz_bayi' | 'analiz_urun' | null
  >(null);
  ...
  async function verileriGetir(
    hedef: 'hepsi' | 'satis' | 'sut' | 'ayar' = 'hepsi'
  ) {
    try {
          ...
          setFisDetay((prev) => {
            ...
            u.forEach((urun) => {
              if (!yeniDetay[urun.id])
                yeniDetay[urun.id] = { adet: '', fiyat: urun.fiyat || '' };
            });
            return yeniDetay;
          });
        }
  ...
  // TAMSAYI GÖSTERİMİ (Küsuratlar Yok)
  const fSayi = (num: any) =>
  ...
  // ÇOKLU SEÇİM YARDIMCISI
  const handleCheckboxToggle = (
    listName: 'ciftlikler' | 'bayiler' | 'urunler',
    stateObj: any,
    setStateFn: any,
    val: string
  ) => {
    setStateFn((prev: any) => {
      ...
      if (arr.includes(val))
        return { ...prev, [listName]: arr.filter((x: string) => x !== val) };
      return { ...prev, [listName]: [...arr, val] };
    });
  };

  // AKILLI TABLO BAŞLIĞI (Yazı Solda, Ok En Sağda, İçerik Tam Altta)
  const Th = ({
    label,
    sortKey,
    currentSort,
            ...
            {filterType && (
              <span
                ...
                style={{
                  fontSize: '10px',
                  padding: '2px',
                  background: isAnaliz ? '#7c3aed' : '#e2e8f0',
                  borderRadius: '4px',
                  display: 'flex',
                  alignItems: 'center',
  ...
  async function handleSutKaydet() {
    if (!sutForm.ciftlik || !sutForm.kg || !sutForm.fiyat)
      return alert('Çiftlik, KG ve Fiyat alanları zorunludur!');
    const p = {
      ...sutForm,
    ...
    const { error } = editingSutId
      ? await supabase.from('sut_giris').update(p).eq('id', editingSutId)
      : await supabase.from('sut_giris').insert(p);
    if (error) return alert('Hata: ' + error.message);
  ...
  const handleBayiSecimi = (secilenBayi: string) => {
    ...
    urunler.forEach((u) => {
      const bayiSatislari = satisList.filter(
        (s) => s.bayi === secilenBayi && s.urun === u.isim
      );
      let hafizaFiyat = u.fiyat || '';
      if (bayiSatislari.length > 0) {
        const sonSatis = bayiSatislari.sort(
  ...
  const eskiBorc = useMemo(() => {
    ...
    const bayiFisleri = satisFisList.filter(
      (f) => f.bayi === fisUst.bayi && f.id !== editingFisId
    );
    return bayiFisleri.reduce(
      (toplam, f) => toplam + Number(f.kalan_bakiye || 0),
      0
    );
  ...
  async function handleTopluFisKaydet() {
    ...
    const eklenecekUrunler = urunler.filter(
      (u) => Number(fisDetay[u.id]?.adet) > 0
    );
    if (eklenecekUrunler.length === 0)
      return alert('Fişte işlem yok! Lütfen en az bir ürüne adet girin.');
    ...
    if (editingFisId) {
      ...
      const { error: errFisUpd } = await supabase
        .from('satis_fisleri')
        .update(fisMaster)
        .eq('id', editingFisId);
      ...
      const { error: errDel } = await supabase
        .from('satis_giris')
        .delete()
        .eq('fis_no', ortakFisNo);
      if (errDel)
        return alert('Eski ürünleri silerken hata: ' + errDel.message);

      const insertArray = eklenecekUrunler.map((u) => {
        const adet = Number(fisDetay[u.id].adet),
          fiyat = Number(fisDetay[u.id].fiyat),
          kgEslesme = u.isim.match(/(\d+(?:\.\d+)?)\s*(kg|lt|l|gr)\b/i);
        return {
          fis_no: ortakFisNo,
          tarih: fisUst.tarih,
          bayi: fisUst.bayi,
          ...
          toplam_kg: (kgEslesme ? Number(kgEslesme[1]) : 1) * adet,
          tutar: adet * fiyat,
          bos_kova: 0,
      ...
      const { error: errDetay } = await supabase
        .from('satis_giris')
        .insert(insertArray);
      if (errDetay) {
        alert(
          'Yeni ürünler kaydedilirken hata oluştu! Eski verileriniz geri yükleniyor...'
        );
        const kurtarilacakVeriler = eskiDetaylar.map((eski) => {
          const { id, ...gerisi } = eski;
          return gerisi;
        });
      ...
      const { error: errFisIns } = await supabase
        .from('satis_fisleri')
        .insert(fisMaster);
    ...
    const fisGosterimData = {
      ...
      urunler: eklenecekUrunler.map((u) => ({
        isim: u.isim,
        adet: Number(fisDetay[u.id].adet),
        fiyat: Number(fisDetay[u.id].fiyat),
        tutar: Number(fisDetay[u.id].adet) * Number(fisDetay[u.id].fiyat),
      })),
  ...
  const resetFisForm = () => {
    ...
    urunler.forEach(
      (u) => (temizDetay[u.id] = { adet: '', fiyat: u.fiyat || '' })
    );
  ...
  const handleFisDuzenle = (fis: SatisFis) => {
    ...
    if (safAciklama.includes('] - '))
      safAciklama = safAciklama.split('] - ')[1];
  ...
  const handleFisDetayGoster = (fis: SatisFis) => {
    const ilgiliUrunler = satisList.filter((s) => s.fis_no === fis.fis_no);
    const bayiFisleri = satisFisList.filter(
      (f) => f.bayi === fis.bayi && f.tarih <= fis.tarih && f.id !== fis.id
    );
    const oGunkuEskiBorc = bayiFisleri.reduce(
      (toplam, f) => toplam + Number(f.kalan_bakiye || 0),
      0
    );
  ...
  async function handleFisSil(fis: SatisFis) {
    if (
      !confirm(
        `Tüm fişi (${fis.fis_no}) ve içindeki ürünleri silmek istediğinize emin misiniz?`
      )
    )
      return;
  ...
  const handleResimPaylas = () => {
    ...
    if (typeof (window as any).html2canvas !== 'undefined') {
      (window as any)
        .html2canvas(fisElement, { scale: 3, backgroundColor: '#ffffff' })
        .then((canvas: any) => {
          canvas.toBlob(
            (blob: Blob | null) => {
              if (!blob) return;
              const file = new File([blob], `Sultankoy_Fis_${Date.now()}.jpg`, {
                type: 'image/jpeg',
              });
              if (
                navigator.share &&
                ...
                navigator
                  .share({ title: 'Sultanköy Fiş Özeti', files: [file] })
                  .catch(() => {});
              else {
  ...
  const handleWhatsappGonder = () => {
    ...
    text += `Tarih: ${sonFisData.tarih
      .split('-')
      .reverse()
      .join('.')}\nSayın: *${sonFisData.bayi}*\n--------------------------\n`;
    sonFisData.urunler.forEach((u: any) => {
      text += `${u.isim}: ${u.adet} x ${fSayi(u.fiyat)} = *${fSayi(
        u.tutar
      )}*\n`;
    });
    text += `--------------------------\n*GENEL TOPLAM: ${fSayi(
      sonFisData.genelToplam
  ...
  let fFisList = sortData(
    satisFisList.filter(
      (f: any) =>
        (fisFiltre.bayiler.length === 0 ||
          fisFiltre.bayiler.includes(f.bayi)) &&
        (!fisFiltre.baslangic || f.tarih >= fisFiltre.baslangic) &&
        (!fisFiltre.bitis || f.tarih <= fisFiltre.bitis)
    ),
    fisSort
  );
  const tFisToplam = fFisList.reduce(
    (a: number, b: any) => a + Number(b.toplam_tutar),
    0
  );
  const tFisTahsilat = fFisList.reduce(
    (a: number, b: any) => a + Number(b.tahsilat),
    0
  );
  const tFisKalan = fFisList.reduce(
    (a: number, b: any) => a + Number(b.kalan_bakiye),
    0
  );

  let fSutList = sortData(
    sutList.filter(
      (s: any) =>
        (sutFiltre.ciftlikler.length === 0 ||
          sutFiltre.ciftlikler.includes(s.ciftlik)) &&
  ...
  const tSutTl = fSutList.reduce(
    (a: number, b: any) => a + Number(b.toplam_tl),
    0
  );

  let fAnalizList = sortData(
    satisList.filter(
      (s: any) =>
        (analizFiltre.bayiler.length === 0 ||
          analizFiltre.bayiler.includes(s.bayi)) &&
        (analizFiltre.urunler.length === 0 ||
          analizFiltre.urunler.includes(s.urun)) &&
        (!analizFiltre.baslangic || s.tarih >= analizFiltre.baslangic) &&
        (!analizFiltre.bitis || s.tarih <= analizFiltre.bitis)
    ),
    analizSort
  );
  const tAnalizKg = fAnalizList.reduce(
    (a: number, b: any) => a + Number(b.toplam_kg),
    0
  );
  const tAnalizTutar = fAnalizList.reduce(
    (a: number, b: any) => a + Number(b.tutar),
    0
  );
  ...
  const renderOzet = () => (
    <div className="tab-fade-in main-content-area">
      ...
      >
        <div className="card" style={{ borderLeft: `5px solid ${temaRengi}` }}>
          <small>Süt Alımı</small>
          <h2 style={{ margin: '5px 0', color: temaRengi }}>
            {fSayi(sutList.reduce((a, b) => a + Number(b.kg), 0))} KG
          </h2>
        </div>
        <div className="card" style={{ borderLeft: '5px solid #059669' }}>
          <small>Toplam Satış</small>
          <h2 style={{ margin: '5px 0', color: '#059669' }}>
            {fSayi(
              satisFisList.reduce((a, b) => a + Number(b.toplam_tutar), 0)
            )}
          </h2>
        </div>
      </div>
    ...
    <div className="tab-fade-in main-content-area">
      ...
      <div className="compact-totals">
        <div className="c-kutu" style={{ borderLeftColor: temaRengi }}>
          <span>SÜT (KG)</span>
          <b style={{ color: temaRengi }}>{fSayi(tSutKg)}</b>
        </div>
        <div className="c-kutu" style={{ borderLeftColor: temaRengi }}>
          <span>TOPLAM TUTAR</span>
          <b style={{ color: temaRengi }}>{fSayi(tSutTl)}</b>
        </div>
      </div>
            ...
            <tr>
              ...
              >
                <div style={{ display: 'flex', gap: '4px' }}>
                  <input
                    type="date"
                    className="date-icon-only"
                    title="Başlangıç"
                    value={sutFiltre.baslangic}
                    onChange={(e) =>
                      setSutFiltre({ ...sutFiltre, baslangic: e.target.value })
                    }
                  />
                  <input
                    type="date"
                    className="date-icon-only"
                    title="Bitiş"
                    value={sutFiltre.bitis}
                    onChange={(e) =>
                      setSutFiltre({ ...sutFiltre, bitis: e.target.value })
                    }
                  />
                </div>
              </Th>
              ...
              >
                <div style={{ display: 'flex', gap: '4px' }}>
                  <input
                    ...
                    className="date-icon-only"
                    title="Başlangıç"
                    value={fisFiltre.baslangic}
                    onChange={(e) =>
                      setFisFiltre({ ...fisFiltre, baslangic: e.target.value })
                    }
                  />
                  <input
                    type="date"
                    className="date-icon-only"
                    title="Bitiş"
                    value={fisFiltre.bitis}
                    onChange={(e) =>
                      setFisFiltre({ ...fisFiltre, bitis: e.target.value })
                    }
                  />
                </div>
              </Th>
            ...
            {fFisList.map((f) => (
              <tr key={f.id}>
                ...
                <td
                  style={{
                    textAlign: 'right',
                    color: f.kalan_bakiye > 0 ? '#dc2626' : '#64748b',
                    fontWeight: 'bold',
                  }}
                >
                  {fSayi(f.kalan_bakiye)}
                </td>
                <td className="actions-cell">
                  <div className="action-buttons">
                    <button
                    ...
                    <button
                      onClick={() => handleFisDuzenle(f)}
                      className="ed-btn"
                    >
                      ✏️
                    </button>
                    <button onClick={() => handleFisSil(f)} className="dl-btn">
                      ✕
                    </button>
                  </div>
                </td>
          ...
          <thead>
            <tr>
              <Th
                label="TARİH"
                sortKey="tarih"
                currentSort={analizSort}
                setSort={setAnalizSort}
                isAnaliz={true}
              >
                <div style={{ display: 'flex', gap: '4px' }}>
                  <input
          ...
          {sec.open && (
            ...
            >
              <div className="responsive-form" style={{ marginBottom: '10px' }}>
                ...
                <button
                  onClick={() =>
                    ayarIslem(sec.t, sec.val, 'ekle', null, sec.setVal)
                  }
              ...
              <div className="responsive-form" style={{ marginTop: '5px' }}>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (!sId) return;
                    const targetItem = sec.data.find(
                      (d) => String(d.id) === String(sId)
                    );
                    if (confirm(`"${targetItem?.isim}" silinecek?`)) {
                ...
                >
                  <option value="">
                    Kayıtlı Listeyi Gör / Silmek İçin Seçin...
                  </option>
                  {sec.data.map((d) => (
                    <option
                      key={d.id}
                    ...
                    >
                      {d.isim}{' '}
                      {sec.isUrun && (d as Urun).fiyat
                        ? `(${fSayi((d as Urun).fiyat)} ₺)`
                        : ''}
                    </option>
                  ))}
                </select>
  ...
  if (!session) {
    return (
      ...
      >
        ...
        >
          <h2
            style={{ margin: '0 0 8px', color: '#0f172a', textAlign: 'center' }}
          >
            Sultanköy v2
          </h2>
          <p
          ...
          >
            <input
              type="checkbox"
              id="remember"
              defaultChecked={!!localStorage.getItem('user')}
            />{' '}
            Beni Hatırla
          </label>
          <button
    ...
    <div className="app-container" style={{ fontSize: `${fontSize}px` }}>
                ...
                {activeFilterModal === 'fis_bayi' &&
                  bayiler.map((b) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={fisFiltre.bayiler.includes(b.isim)}
                        onChange={() =>
                          handleCheckboxToggle(
                            'bayiler',
                            fisFiltre,
                ...
                {activeFilterModal === 'analiz_bayi' &&
                  bayiler.map((b) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={analizFiltre.bayiler.includes(b.isim)}
                        onChange={() =>
                          handleCheckboxToggle(
                            'bayiler',
                            analizFiltre,
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '15px' }}>
                <button
                  onClick={() => {
                    if (activeFilterModal === 'sut_ciftlik')
                      setSutFiltre({ ...sutFiltre, ciftlikler: [] });
                    if (activeFilterModal === 'fis_bayi')
                      setFisFiltre({ ...fisFiltre, bayiler: [] });
                    if (activeFilterModal === 'analiz_bayi')
                      setAnalizFiltre({ ...analizFiltre, bayiler: [] });
                    if (activeFilterModal === 'analiz_urun')
                      setAnalizFiltre({ ...analizFiltre, urunler: [] });
                  }}
                  style={{
                    flex: 1,
                    padding: '10px',
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '10px',
                    background: activeFilterModal.includes('analiz')
                      ? '#8b5cf6'
                      : temaRengi,
                    color: '#fff',
                    border: 'none',
            ...
            >
              <div
                style={{
                  ...
                  justifyContent: 'space-between',
                  alignItems: 'center',
                  background: editingSutId ? '#fef3c7' : '#f8fafc',
              ...
              >
                <h3
                  style={{
                    margin: 0,
                    color: editingSutId ? '#b45309' : temaRengi,
                    fontSize: '15px',
                  }}
                >
                  {editingSutId
                    ? '✏️ Süt Kaydını Düzenle'
                    : '🥛 Yeni Süt Girişi'}
                </h3>
                <button
                  onClick={() => setIsSutModalOpen(false)}
                  style={{
                    background: 'none',
                    border: 'none',
                    fontSize: '20px',
              ...
              >
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input
                    ...
                    value={sutForm.tarih}
                    onChange={(e) =>
                      setSutForm({ ...sutForm, tarih: e.target.value })
                    }
                    className="m-inp"
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSutForm({ ...sutForm, ciftlik: e.target.value })
                    }
                    className="m-inp"
                    style={{ flex: 2, fontWeight: 'bold', padding: '8px' }}
                  >
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      value={sutForm.kg}
                      onChange={(e) =>
                        setSutForm({ ...sutForm, kg: e.target.value })
                      }
                      className="m-inp"
                      style={{
                        width: '100%',
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      value={sutForm.fiyat}
                      onChange={(e) =>
                        setSutForm({ ...sutForm, fiyat: e.target.value })
                      }
                      className="m-inp"
                      style={{
                        width: '100%',
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSutForm({ ...sutForm, aciklama: e.target.value })
                    }
                    className="m-inp"
              ...
              >
                ...
                >
                  ...
                  <b style={{ color: temaRengi, fontSize: '18px' }}>
                    {fSayi(
                      (Number(sutForm.kg) || 0) * (Number(sutForm.fiyat) || 0)
                    )}{' '}
                    ₺
                  </b>
                </div>
                <button
                  ...
                  className="p-btn btn-anim"
                  style={{
                    background: editingSutId ? '#f59e0b' : temaRengi,
                    width: '100%',
                    height: '45px',
                    fontSize: '15px',
                  }}
            ...
            >
              <div
                style={{
                  ...
                  justifyContent: 'space-between',
                  alignItems: 'center',
                  background: editingFisId ? '#fef3c7' : '#f8fafc',
              ...
              >
                <h3
                  style={{
                    margin: 0,
                    color: editingFisId ? '#b45309' : '#059669',
                    fontSize: '15px',
                  }}
                >
                ...
                <button
                  onClick={() => setIsFisModalOpen(false)}
                  style={{
                    background: 'none',
                    border: 'none',
                    fontSize: '20px',
              ...
              <div style={{ padding: '10px 12px', overflowY: 'auto', flex: 1 }}>
                ...
                >
                  <input
                    ...
                    value={fisUst.tarih}
                    onChange={(e) =>
                      setFisUst({ ...fisUst, tarih: e.target.value })
                    }
                    className="m-inp"
                    style={{
                      flex: '0 0 100px',
                  ...
                  <select
                    value={fisUst.bayi}
                    onChange={(e) => handleBayiSecimi(e.target.value)}
                    className="m-inp grow-inp"
                    style={{
                      fontWeight: 'bold',
                      padding: '6px 8px',
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <select
                    ...
                    onChange={(e) =>
                      setFisUst({ ...fisUst, odeme_turu: e.target.value })
                    }
                    className="m-inp"
                    style={{
                      flex: '0 0 95px',
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFisUst({ ...fisUst, aciklama: e.target.value })
                    }
                    className="m-inp grow-inp"
                    style={{
                      padding: '6px 8px',
              ...
              >
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFisUst({ ...fisUst, tahsilat: e.target.value })
                    }
                    className="m-inp"
                    style={{
                      flex: '0 0 90px',
                ...
                >
                  <span
                    style={{
                      color:
                        fisCanliToplam - Number(fisUst.tahsilat || 0) > 0
                          ? '#dc2626'
                          : '#059669',
                      fontWeight: 'bold',
                      fontSize: '13px',
                ...
                {aktifBayi && (
                  <>
                    ...
                    >
                      ...
                      <b style={{ color: '#64748b', fontSize: '12px' }}>
                        {fSayi(eskiBorc)}
                      </b>
                    </div>
                    <div
                ...
                <button
                  ...
                  className="p-btn btn-anim"
                  style={{
                    background: editingFisId ? '#f59e0b' : '#059669',
                    width: '100%',
                    height: '40px',
                    fontSize: '14px',
                  }}
        ...
        {sonFisData && (
          ...
          >
            ...
            >
              ...
              >
                <h2
                  style={{ margin: '0 0 2px', color: '#000', fontSize: '18px' }}
                >
                  SULTANKÖY
                </h2>
                <div
                ...
                >
                  <thead>
                    <tr style={{ borderBottom: '1px solid #000' }}>
                      <th style={{ paddingBottom: '4px', textAlign: 'left' }}>
                        Ürün
                      </th>
                      <th style={{ paddingBottom: '4px', textAlign: 'right' }}>
                        Adet
                      </th>
                      <th style={{ paddingBottom: '4px', textAlign: 'right' }}>
                        B.Fiyat
      ...
      <footer className="fixed-nav main-content-area">
        {[
          ...
          <button
            ...
            className={`n-item btn-anim ${
              activeTab === item.id ? 'active' : ''
            }`}
            style={
              activeTab === item.id
                ? {
                    color: item.id === 'analiz' ? '#8b5cf6' : temaRengi,
                    borderTopColor:
                      item.id === 'analiz' ? '#8b5cf6' : temaRengi,
                  }
                : {}
            }
          >
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* BEYAZ TAKVİM İKONLARI - BAŞLIĞIN HEMEN YANINDA */
        ...
        .date-icon-only::-webkit-calendar-picker-indicator { position: absolute; top: 0; left: 0; width: 100%; height: 100%; cursor: pointer; opacity: 0.9; padding: 0; margin: 0; filter: invert(0); transition: opacity 0.2s; }
